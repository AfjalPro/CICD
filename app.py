...
@app.get("/items/")
async def read_items(current_user=Depends(get_current_user), db=Depends(get_db)):
    return {"items": [ {"id": it.id, "name": it.name} for it in db.query(Item).all() ]}

@app.post("/items/")
async def create_item(name: str, current_user=Depends(get_current_user), db=Depends(get_db)):
    item = Item(name=name)
